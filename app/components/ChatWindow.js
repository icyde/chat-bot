...
/* 
ChatWindow - communicate with live agents on ChatWindow
input: user input from text box
id: message id for mapping
handleSubmit: Submit user message from chatbot to ChatWindow system
...
const ChatWindow = ({
  ...
  useEffect(() => {
    if (conversationId !== "" && socket === null) {
      connectToWebsocket(conversationId);
    }
  }, [connectToWebsocket, conversationId, socket]);
      ...
      >
        <div className="flex flex-col flex-1 justify-between">
          <div>
            ...
            {emailSent === false && conversationId === "" ? (
      ...
      <div className="mt-2">
        <form onSubmit={handleSubmit}>
          <input
            value={input}
            disabled={
              (emailSent === false && conversationId === "") || isTyping
            }
